...
impl Commands {
    ...
    fn get_toml(path: Option<&str>, search_count: Option<u32>) -> Result<Project, String> {
        ...
        Self::get_toml(Some(format!("{}/..", path).as_str()), Some(search_count + 1))
    }
...
",
        ...
        let mut file =
            File::create(&format!("{}/Ocean.toml", project.get_name())).expect("Could not create Ocean.toml");
...
Option:
    ...
    pub fn set_data_platform(args: &[String], platform_name: String) -> Result<(), String> {
        let mut project = Self::get_toml(None, None)?;
...
Option:
    ...
    pub fn get_data_platform(args: &[String], platform_name: String) -> Result<(), String> {
        let project = Self::get_toml(None, None)?;
